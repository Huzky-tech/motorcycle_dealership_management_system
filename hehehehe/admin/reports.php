...
$sql = "SELECT i.id, i.customer_name, 
               COALESCE(i.model, m.model) AS model, 
               i.color, i.inquiry_details, i.purchase_date, 
               IFNULL(m.srp, 0) AS srp, IFNULL(m.downpayment, 0) AS downpayment, 
               IFNULL(m.monthly, 0) AS monthly
        FROM inquiries i
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_id'])) {
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            echo "<script>alert('Order deleted successfully!'); window.location.href='admin/dashboard.php';</script>";
        } else {
